...
--------1 Song Stuff -------------
coderUser = 'FridayDayFunkin, Fanta' -- if its only this team then put this
composerUser = 'Gavin360, _KozinDeadlight, Ponky, FridayDayFunkin'
artistUser = 'PranksterBlocky, FridayDayFunkin, Fanta'
charterUser = 'FridayDayFunkin, Bol'
--------------------------------
--------Colors-----------------
mainColor =  '222222' -- Big Box Color
secondaryColor = 'F8DFC1' -- Lines Color
...
function onCreate()
 if enabled == true then
  ...
  if thinlines ~= true then
    ---LEFT---
    makeLuaSprite('lineLeft', 'lineLeft',  -700, 1300)
    makeGraphic("lineLeft", -700, 1300, secondaryColor)
    setObjectCamera('lineLeft', 'other')
    setProperty('lineLeft.alpha', 1)
    setProperty('lineLeft.y', -700)
    setProperty('lineLeft.x', 348)
    addLuaSprite('lineLeft', true)
       ---RIGHT--- 
    makeLuaSprite('lineRight', 'lineRight',  -500, 1300)
    makeGraphic("lineRight", 40, 1300, secondaryColor)
    setObjectCamera('lineRight', 'other')
    setProperty('lineRight.alpha', 1)
  ...
  if thinlines == true then
    ---LEFT---
    makeLuaSprite('lineLeft', 'lineLeft',  -500, 1300)
    makeGraphic("lineLeft", 10, 1300, secondaryColor)
    setObjectCamera('lineLeft', 'other')
    ...
    addLuaSprite('lineLeft', true)
       ---RIGHT--- 
    makeLuaSprite('lineRight', 'lineRight',  -500, 1300)
    makeGraphic("lineRight", 10, 1300, secondaryColor)
    setObjectCamera('lineRight', 'other')
    ...
    addLuaSprite('lineRight', true)
       ...
       ---LINES---
    makeLuaText("CREDITSthing", "CREDITS", 0, 0, -1000)
    makeLuaText("CODERthing", "CODER", 0, 0, -1000)
    makeLuaText("COMPOSERthing", "COMPOSER", 0, 0, -1000)
    makeLuaText("ARTISTthing", "ARTIST", 0, 0, -1000)
    makeLuaText("CHARTERthing", "CHARTER", 0, 0, -1000)
    ...
    setObjectCamera('CHARTER', 'other')
        ...
        -- CODER --
    makeLuaText("CODER", coderUser, -500, 0, -1000)
    setObjectCamera('CODER', 'other')
    addLuaText('CODER')

        -- COMPOSER --
    makeLuaText("COMPOSER", composerUser, 0, 0, -1000)
    setObjectCamera('COMPOSER', 'other')
    addLuaText('COMPOSER')

        -- ARTIST --
    makeLuaText("ARTIST", artistUser, 0, 0, -1000)
    setObjectCamera('ARTIST', 'other')
    addLuaText('ARTIST')

         -- CHARTER --
    makeLuaText("CHARTER", charterUser, 0, 0, -1000)
    setObjectCamera('CHARTER', 'other')
    addLuaText('CHARTER')
